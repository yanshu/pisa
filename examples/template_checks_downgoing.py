...
def get_up_map(maps,flavs,iType=None):
    ...
    half_czbin = (czbin_edges-1)/2
    print 'nczbin = ', len(map['cscd']['czbins'])
    if iType is None:
        return {flav:{
            'map': map[flav]['map'][:,0:czbin_edges/2],
            'ebins':map[flav]['ebins'],
            'czbins': map[flav]['czbins'][0:11] }
                for flav in flavs}
    else:
        return {flav:{
            'map': map[flav][iType]['map'][:,0:10],
            'ebins':map[flav][iType]['ebins'],
            'czbins': map[flav][iType]['czbins'][0:11] }
                for flav in flavs}

def get_down_map(maps,flavs,iType=None):
    ''' Gets the downgoing map and flip it.'''
    map = maps[4]   #use the last stage
    if iType is None:
        return {flav:{
            'map': np.fliplr(map[flav]['map'][:,10:]),
            'ebins':map[flav]['ebins'],
            'czbins': np.sort(-map['trck']['czbins'][10:]) }
                for flav in flavs}
    else:
        return {flav:{
            'map': np.fliplr(map[flav][iType]['map'][:,10:]), 
            'ebins':map[flav][iType]['ebins'],
            'czbins': np.sort(-map['trck'][iType]['czbins'][10:]) }
                for flav in flavs}
...
settings file. ''')
    parser.add_argument('template_settings',metavar='JSON',
                        help='Settings file to use for upgoing template generation')
    ...
    nutau_down = get_down_map(nutau,['trck','cscd']) 
    no_nutau_up = get_up_map(no_nutau,['trck','cscd']) 
    no_nutau_down = get_down_map(no_nutau,['trck','cscd']) 

    #ratio_nutau = get_ratio_2D(nutau_up,nutau_down,['trck','cscd']) 
    #ratio_no_nutau = get_ratio_2D(no_nutau_up,no_nutau_down,['trck','cscd']) 
    #ratio_of_2D_ratios = get_ratio_2D(ratio_nutau,ratio_no_nutau,['trck','cscd'])
    ratio_nutau = get_residual(nutau_up,nutau_down,['trck','cscd']) 
    ratio_no_nutau = get_residual(no_nutau_up,no_nutau_down,['trck','cscd']) 

    #ratio_of_2D_ratios = get_ratio_2D(ratio_nutau,ratio_no_nutau,['trck','cscd'])
    #plt.figure(figsize=(5,10))
    #plt.subplot(2,1,1)
    #show_map(ratio_of_2D_ratios['trck'])
    #plt.title('ratio_of_ratios trck counts',fontsize='large')
    #plt.subplot(2,1,2)
    #show_map(ratio_of_2D_ratios['cscd'])
    #plt.title('ratio_of_ratios cscd counts',fontsize='large')
    #plt.savefig(args.outdir+'/ratio/'+'ratio_of_ratios_2D.png')
    ...
    if type(ratio_nutau) is tuple:
        plot_stages(ratio_nutau,ratio_no_nutau,title=args.title,save=args.save,outdir=args.outdir+'ratio/')
    else:
        plot_pid_stage(ratio_nutau,ratio_no_nutau,title=args.title,save=args.save,outdir=args.outdir+'ratio/',log=False)
