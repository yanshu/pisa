...
# Define custom build order, so that the python interface module created by
# SWIG is staged in build_py.
class build(_build):
...
if OPENMP:
    gaussians_module = Extension(
        name='pisa.utils.gaussians',
        sources=['pisa/utils/gaussians.pyx'],
        libraries=['m'],
