...
def get_fisher_matrices(template_settings, grid_settings, IMH=True, NMH=False,
                        ...
                        outdir=None):
    """Run the Fisher analysis for the chosen hierarchy(ies) (inverted by
    default).
    ...
    If save_templates=True and no hierarchy is given, only fiducial templates
    will be written out; if one is given, then the templates used to obtain the
    gradients will be written out in addition.
    """
    if outdir is None and (save_templates or dump_all_stages):
        logging.info('No output directory specified. Will save templates to'
    ...
    if chosen_data == []:
        # In this case, only the fiducial maps (for both hierarchies) will be
        # written
        logging.info('No Fisher matrices will be built.')

    # There is no sense in performing any of the following steps if no Fisher
    # matrices are to be built and no templates are to be saved.
    if chosen_data!=[] or dump_all_stages or save_templates:

        # Initialise return dict to hold Fisher matrices
        fisher = {data_tag:{'cscd':[], 'trck':[], 'comb':[]}
        ...
        # Generate fiducial templates for both hierarchies (needed for partial
        # derivatives w.r.t. hierarchy parameter)
        fiducial_maps = {}
        for hierarchy in ['NMH', 'IMH']:
            ...
            # Generate fiducial maps, either all of them or only the ultimate
            # one
            ...
            if dump_all_stages:
                fiducial_maps[hierarchy] = fid_maps['final_event_rate']
            else:
                fiducial_maps[hierarchy] = fid_maps

            # save fiducial map(s)
            # all stages
            if dump_all_stages:
                stage_maps = {
                    '0_unoscillated_flux': fid_maps['flux_maps'],
                    '1_oscillated_flux': fid_maps['osc_flux_maps'],
                    '2_oscillated_counts': fid_maps['event_rate_maps'],
                    '3_reco': fid_maps['event_rate_reco_maps'],
                    '4_pid': fid_maps['final_event_rate']
                }
                logging.info('Writing fiducial maps (all stages) for %s to %s.'
                             % (hierarchy, outdir))
                to_json(stage_maps,
                        os.path.join(outdir, 'fid_map_' + hierarchy + '.json'))
            # only the final stage
            elif save_templates:
                logging.info('Writing fiducial map (final stage) for %s to %s.'
                             % (hierarchy, outdir))
        ...
        # Calculate Fisher matrices for the user-defined cases (NHM true and/or
        # IMH true)
        for data_tag, data_normal in chosen_data:
            logging.info('Running Fisher analysis for %s.' % (data_tag))

            # The fiducial params are selected from the hierarchy case that
            # does NOT match the data, as we are varying from this model to
            # find the 'best fit'
            fiducial_params = select_hierarchy(params, not data_normal)

            # Get the free parameters (i.e. those for which the gradients
            ...
            # If Fisher matrices exist for both channels, add the matrices to
            # obtain the combined one.
            if len(fisher[data_tag].keys()) > 1:
                fisher[data_tag]['comb'] = FisherMatrix(
                    ...
                    best_fits=[fiducial_params[par]['value']
                               for par in gradient_maps.keys()],
                    priors=[Prior.from_param(fiducial_params[par])
                            for par in gradient_maps.keys()],
                )
        return fisher
...
if __name__ == '__main__':
    parser = ArgumentParser(
        description='''Runs the Fisher analysis method by varying a number of
        systematic parameters defined in a settings.json file, taking the
        number of test points from a grid_settings.json file, and saves the
    ...
    for data_tag in fisher_matrices:
        ...
        for chan in fisher_matrices[data_tag]:
            if chan == 'comb':
                ...
                outfile = os.path.join(args.outdir, fisher_basename +
                                       '_%s.json' % chan)
