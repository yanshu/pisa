...
parser.add_argument('--use_curvefit',action='store_true',default=False,
                    help="Use np.curve_fit, when using bin count, the slope is divided by baseline bin count and saved to json file.")
parser.add_argument('--use_polyfit',action='store_true',default=False,
                    help="Use np.polyfit, when using bin count, the slope is divided by baseline bin count and saved to json file.")
...
if args.use_event_PISA:
    ...
    pisa_mode = 'hist'
# read template_settings
template_settings = from_json(args.template_settings)
czbin_edges = template_settings['binning']['czbins']
ebin_edges = template_settings['binning']['anlys_ebins']
channel = template_settings['params']['channel']['value']

if not(args.use_curvefit or args.use_polyfit):
    parser.error('Please choose the fitting tool (for linear fit): use_curvefit or use_polyfit')
fit_bin_ratio = template_settings['params']['fit_bin_ratio']['value']
fit_thru_baseline = template_settings['params']['fit_thru_baseline']['value']
fitting_mode = ''
if args.use_curvefit:
    fitting_mode += 'curvefit_'
if args.use_polyfit:
    fitting_mode += 'polyfit_'

if fit_bin_ratio:
    fitting_mode += 'bin_ratio_'
else:
    fitting_mode += 'bin_count_'

if fit_thru_baseline:
...
if not args.templ_already_saved:
    ...
    for run_num in run_list:
        ...
        if args.sim == '5digit':
            ...
            aeff_mc_file = 'aeff/events__deepcore__ic86__runs_12%s-16%s:200__proc_v4digit__unjoined_with_fluxes.hdf5' % (run_num,run_num)
        ...
        # Get MC events map 
        MCMap = GetMCError(get_values(select_hierarchy(DH_template_settings['params'], use_NMH)), DH_template_settings['binning']['anlys_ebins'], DH_template_settings['binning']['czbins'], reco_mc_file)
...
for flav in ['trck','cscd']:
    ...
    if args.sim != 'dima_p2':
        ...
        for i in range(0,n_ebins):
            for j in range(0,n_czbins):
                ...
                exec('def dom_eff_linear(x, k, b): return k * x + b')
                if args.sim == '4digit':
                ...
                if args.use_curvefit:
                    if fit_thru_baseline:
                        if fit_bin_ratio:
                            ...
                            popt_1, pcov_1 = curve_fit(dom_eff_linear_through_point, dom_eff_values, bin_counts, sigma=sigma)
                            k1 = popt_1[0]/nominal_bin_counts
                    else:
                        if fit_bin_ratio:
                            popt_1, pcov_1 = curve_fit(dom_eff_linear, dom_eff_values, bin_ratio_values, sigma=sigma)
                            k1 = popt_1[0]
                        else:
                            popt_1, pcov_1 = curve_fit(dom_eff_linear, dom_eff_values, bin_counts, sigma=sigma)
                            k1 = popt_1[0]/nominal_bin_counts
    ...
    for i in range(0,n_ebins):
        for j in range(0,n_czbins):
            ...
            exec('def hole_ice_linear(x, k, b): return k * x + b')
            if args.sim == '4digit':
            ...
            if args.use_curvefit:
                if fit_thru_baseline:
                    if fit_bin_ratio:
                        ...
                        popt_1, pcov_1 = curve_fit(hole_ice_linear_through_point, hole_ice_values, bin_counts, sigma=sigma)
                        k1 = popt_1[0]/nominal_bin_counts
                else:
                    if fit_bin_ratio:
                        popt_1, pcov_1 = curve_fit(hole_ice_linear, hole_ice_values, bin_ratio_values, sigma=sigma)
                        k1 = popt_1[0]
                    else:
                        popt_1, pcov_1 = curve_fit(hole_ice_linear, hole_ice_values, bin_counts, sigma=sigma)
                        k1 = popt_1[0]/nominal_bin_counts
            ...
            if args.plot:
                ...
                if args.use_curvefit and fit_bin_ratio:
