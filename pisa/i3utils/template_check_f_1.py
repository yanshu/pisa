...
from pisa.utils.plot import show_map, sum_map, ratio_map
...
if __name__ == '__main__':
    ...
    plt.figure()
    show_map(nominal_nutau_up_and_down_cscd,vmax=np.max(nominal_nutau_up_and_down_cscd['map'])+10,logE=args.logE)
    if args.save:
        filename = os.path.join(args.outdir,args.title+ '_%s_yr_bg_scale_%s_NutauCCNorm_1_up_down_combined_' % (args.y, args.bg_scale) + 'cscd_5.6_56GeV.png')
        plt.title(r'${\rm %s \, yr \, cscd \, (Nevts: \, %.1f, \, bg \, scale \, %s) }$'%(args.y, np.sum(nominal_nutau_up_and_down_cscd['map']), args.bg_scale), fontsize='large')
        plt.savefig(filename,dpi=150)
        plt.clf()

    plt.figure()
    show_map(nominal_nutau_up_and_down_trck,vmax=np.max(nominal_nutau_up_and_down_trck['map'])+10,logE=args.logE)
    if args.save:
        filename = os.path.join(args.outdir,args.title+ '_%s_yr_bg_scale_%s_NutauCCNorm_1_up_down_combined_' % (args.y, args.bg_scale) + 'trck_5.6_56GeV.png')
        plt.title(r'${\rm %s \, yr \, trck \, (Nevts: \, %.1f, \, bg \, scale \, %s) }$'%(args.y, np.sum(nominal_nutau_up_and_down_trck['map']), args.bg_scale), fontsize='large')
        plt.savefig(filename,dpi=150)
        plt.clf()
