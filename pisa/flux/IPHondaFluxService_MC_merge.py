...
class IPHondaFluxService():
    ...
    def get_flux(self, ebins, czbins, prim, event_by_event):
        ...
        # input prim has to be either a list of strings or a single string; when it's a list of strings,
        # its length has to be the same as the length of input ebins and czbins.
        if type(prim)==list:
        ...
        for idx, energyval in enumerate(evals):
            ...
            for czkey in np.linspace(-0.95,0.95,20):
                ...
                spline_vals.append(splev(logenergyval,self.spline_dict[flux_prim]['%.2f'%czkey],der=1)*0.05)
            int_spline_vals = []
            tot_val = 0.0
            ...
            if event_by_event:
                czfluxes = splev(czvals[idx],spline,der=1)*0.1/energyval
            else:
