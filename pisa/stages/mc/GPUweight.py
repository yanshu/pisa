...
class GPUweight(object):
    ...
    def __init__(self):
	    ...
	    fType e1max_e  = 2.5;
	    fType e2max_e  = 10;
	    fType e1max_mu_e = 0.62;
	    fType e2max_mu_e = 11.45;
	    // Evaluated at
	    fType x1e = 0.5;
	    fType x2e = 3.;
	    ...
	    fType z1max_e  = 0.3;
	    fType z2max_e  = 5.;
	    fType nue_cutoff  = 650.;
	    fType numu_cutoff = 1000.;
	    // Evaluated at
	    fType x1z = 0.5
	    fType x2z = 2.

            __device__ fType LogLogParam(fType energy, fType y1, fType y2, fType x1, fType x2){
                fType nu_nubar = sign(y2);
                if (nu_nubar == 0.0) nu_nubar = 1.;
                y1 = sign(y1)*log10(abs(y1)+0.0001);
                y2 = log10(abs(y2+0.0001));
                modification = pow(nu_nubar*10,(((y2-y1)/(x2-x1))*(log10(energy)-x1)+y1-2.));
                return modification;
            }
            __device__ fType LogLogParam(fType energy, fType y1, fType y2, fType x1, fType x2, fType cutoff_value){
                fType modification = LogLogParam(energy, y1, y2, x1, x2);
                modification *= exp(-1.*energy/cutoff_value);
                return modification;
            }
		...
		A_ave = LogLogParam(energy, e1max_mu*e1, e2max_mu*e2, x1e, x2e);
		A_shape = 2.5*LogLogParam(energy, z1max_mu*z1, z2max_mu*z2, x1z, x2z, numu_cutoff);
		...
		A_ave = LogLogParam(energy, e1max_mu*e1mu + e1max_e*e1e, e2max_mu*e2mu + e2max_e*e2e, x1e, x2e);
		A_shape = 1.*LogLogParam(energy, z1max_mu*z1mu + z1max_e*z1e, z2max_mu*z2mu + z2max_e*z2e, x1z, x2z, nue_cutoff);
		return A_ave - (1.5*norm_fcn(czenith, A_shape, 0.4) - 0.7*A_shape);
            ...
            __device__ fType modRatioUpHor(const int kFlav, fType true_energy, fType true_coszen, fType uphor) {
                fType A_shape;
                if (kFlav == 0){
                    A_shape = 1.*abs(uphor)*LogLogParam(true_e, (z1max_e+z1max_mu),(z2max_e+z2max_mu),x1z, x2z, nue_cutoff);
                }
                if (kFlav == 1){
                    A_shape = 1.*abs(uphor)*LogLogParam(energy=true_e, z1max_mu, z2max_mu, x1z, x2z, numu_cutoff);
                }
                return 1-3.5*sign(uphor)*norm_fcn(true_cz, A_shape, 0.35);
            }
