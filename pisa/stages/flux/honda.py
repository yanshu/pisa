...
integral-preserving method that manipulates 1 dimensional splines of
integrated flux.
...
class honda(Stage):
    ...
    Notes
    ----------
    This service reads in a table of flux values and then performs some form
    of interpolation on them. It is named honda.py since it expects that these
    input tables come from the Honda group and their simulations. All of these
    tables can be found at this url:

    http://www.icrr.u-tokyo.ac.jp/~mhonda/

    These are provided for various locations, the list of which now includes 
    the South Pole as of the 2014/15 calculations. The details of this latest
    calculation are published on the ArXiV here:

    http://arxiv.org/abs/1502.03916

    but also in Phys. Rev. D92, 023004 (2015). These tables are provided over 3
    dimensions - neutrino energy, the cosine of the neutrino zenith angle and
    the azimuth of the neutrino. This is what is meant by a 3D flux calculation
    in the context of this service. One also deals with 2D flux calculations 
    where the azimuth is averaged over. This is the recommended set of tables
    to use for the energies one is interested in in IceCube. The Honda group
    also provide 1D flux calculations, where all of the angles are averaged
    over i.e. they are provided solely as a function of energy.
    ...
    1) The first part of the file name will be which group performed the 
       simulation. So for us they should all say honda
    2) The second part will be the year the calculations were published.
    3) The third part will be the location. For example, spl means the South
       Pole and frj means Frejus. There may also be the word 'mountain' here
       since tables have been produced for some detectors with and without
       the large matter above them (mostly a mountain) that purports to shield
       them from cosmic rays.
    4) The fourth part will be the part of the solar cycle they were produced
       for. Solar activity has an effect on the primaries impacting the Earth
       and therefore on the flux of atmospheric neutrinos. This will either be
       solmax or solmin, which refers to solar maximum and solar minimum 
       respectively. Solar maximum means the flux is slightly reduced.
    5) The fifth part will be a label whether the tables are 2D or 3D. In the
       former they will have aa in the name, which means azimuth-averaged. In
       the latter there will be no fifth part. This service has been designed
       to look for this string specifically to validate the input files
       together with the requested output binning.

    All of the tables in PISA can be found in pisa/resources/flux. PISA 
    actively supports the following tables from Honda:
    ...
    * honda-2015-spl-solmin.d
    ...
    and greatest output from the Honda group. The use of azimuth-averaged tables
    is recommended for the energies IceCube works with, but the ability to read
    3D tables has also been included here.
    ...
    * honda-2011-frj-mountain-solmin-aa.d
    ...
    If you access the Honda website linked above you will notice that the
    2012 South Pole files listed here are NOT on that website. This is because
    they were given to the IceCube collaboration as an interim while Honda was
    between major releases of the simulation. They are known to be buggy and
    have problems which have been fixed with the release of the 2014/15 
    simulation. Please only use these tables when reproducing old results
    that also used them. The Frejus file is also included as a legacy since it
    was used for some of the very first PINGU results. It is not buggy per se, 
    but since we have a better alternative please don't use it.
    ...
    them. Currently PISA supports two methods for this with the Honda files:
    ...
    2) 'integral-preserving' - A slower, but more accurate choice.
       ...
       the correct flux up to the boundaries which the bisplrep method does not.
       ...
       the bin and NOT the value at the bin center. The splining is done using 
       the scipy.interpolate.splrep function.

       In any case, these splines as a function of energy are of the integral of
       ...
       dimension (this time of just flux since it is stable enough and also 
       makes no sense to multiply by cosZenith since we will get sign switches)
       and evaluates the derivative at the required cosZenith value.  

       This method only works when the binning is constant. Thus, it must 
    ...
    To choose between these, set 'flux_mode' in your pipeline settings file
    to either 'bisplrep' or 'integral-preserving'. Setting it to the latter is 
    slower, but the recommended choice.
    """
