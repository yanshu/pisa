...
if __name__ == '__main__':
    ...
    parser.add_argument('--ignore-gpu', action='store_true', default=False,
                        help='''Do not run the gpu examples. You will need to
                        flag this if your system does not have a gpu else it 
                        will fail.''')
    parser.add_argument('--ignore-xsec', action='store_true', default=False,
                        help='''Do not run the gpu examples. You will need to
                        flag this if your system does not have an installation 
    ...
    example_directory = os.path.join(
        'settings','pipeline','examples'
    )
    example_directory = find_resource(example_directory)
    settings_files = glob.glob(example_directory+"/*.cfg")
    ...
    for settings_file in settings_files:
        try:
            ...
            raise ValueError('%s does not work. Please review the error '
                             'message above and fix the problem.'%settings_file)
