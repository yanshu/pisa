        ...
        for name in map_names:
            ...
            for d in range(degree):
                for p in range(len(binning['pid'])):
                    #outputs[name][...,d] = np.swapaxes(gaussian_filter(outputs[name][...,d],sigma=1),split_idx, tot)
                    outputs[name][...,p,d] = gaussian_filter(np.swapaxes(outputs[name],split_idx, tot)[...,p,d],sigma=1)
