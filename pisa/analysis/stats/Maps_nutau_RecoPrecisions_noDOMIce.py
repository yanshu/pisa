...
def get_pseudo_data_fmap(template_maker, fiducial_params, channel, seed=None):
    ...
    if fiducial_params['residual_up_down'] or fiducial_params['simp_up_down'] or fiducial_params['ratio_up_down']:
        ...
        [template_up_prcs,reflected_template_down_prcs] = apply_reco_precisions([template_up,reflected_template_down],fiducial_params)
        true_fmap_up = Maps.flatten_map(template_up_prcs, channel=fiducial_params['channel'])
        true_fmap_down = Maps.flatten_map(reflected_template_down_prcs, channel=fiducial_params['channel'])
        ...
        true_template = template_maker.get_template(fiducial_params)
        # add domeff and/or hole ice effects
        true_template_prcs = apply_reco_precisions(true_template, fiducial_params)
        true_fmap = Maps.flatten_map(true_template_prcs, channel=channel)
...
def get_true_template(template_params, template_maker):
    if template_params['theta23'] == 0.0:
        ...
        # add domeff and/or hole ice effects
        true_template_prcs = apply_reco_precisions(true_template, template_params)
        true_fmap = Maps.flatten_map(true_template_prcs, channel=template_params['channel'])
        ...
        # add domeff and/or hole ice effects
        [template_up_prcs,reflected_template_down_prcs] = apply_reco_precisions([template_up,reflected_template_down],template_params)
        true_fmap_up = Maps.flatten_map(template_up_prcs, channel=template_params['channel'])
        true_fmap_down = Maps.flatten_map(reflected_template_down_prcs, channel=template_params['channel'])
